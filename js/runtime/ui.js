...
export default class UI {
  ...
  touchend (e) {
    if (e.changedTouches[0].identifier === Databus.touchpos.Id) {
      const h = e.changedTouches[0].clientY - Databus.touchpos.y
      const w = e.changedTouches[0].clientX - Databus.touchpos.x
      if (Math.abs(h) > 20) {
        direction = (h > 0) ? -1 : 1
  ...
  pausetouch (e) {
    ...
    const height = width / CONTAINER_WIDTH * CONTINUE_HEIGHT
    const left = (screenWidth - width) / 2
    if ((e.clientX > left) &&
